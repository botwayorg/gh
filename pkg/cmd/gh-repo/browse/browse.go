...
func NewCmdBrowse(f *cmdutil.Factory, runF func(*BrowseOptions) error) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: heredoc.Doc(`
			$ botway gh-repo browse
			#=> Open the home page of the current repository
			$ botway gh-repo browse 61
			#=> Open issue or pull request 61
			$ botway gh-repo browse --settings
			#=> Open repository settings
			$ botway gh-repo browse main.go:312
			#=> Open main.go at line 312
			$ botway gh-repo browse main.go --branch main
...
func parseFileArg(fileArg string) (string, error) {
	...
	if len(arr) > 2 {
		return "", fmt.Errorf("invalid use of colon\nUse 'botway gh-repo browse --help' for more information about browse\n")
	}
